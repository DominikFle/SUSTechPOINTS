...
def sample_one_obj(points, num):
    if points.shape[0] < NUM_POINT:
        return np.concatenate(
            [points, np.zeros((NUM_POINT - points.shape[0], 3), dtype=np.float32)],
            axis=0,
        )
    else:
...
def predict_yaw(points):
    ...
    input_data = np.stack(
        [
            x
            for x in map(
                lambda x: sample_one_obj(points, NUM_POINT), range(RESAMPLE_NUM)
            )
        ],
        axis=0,
    )
...
if False:
    ...
    def sample_one_obj(points, num):
        ...
        if points.shape[0] > num:
            ...
            sample_idx = np.random.randint(
                0, high=points.shape[0], size=num - points.shape[0]
            )
    ...
    def filter_candidate_objects(clusters):
        ...
        input_cluster_points = np.stack(
            [sample_one_obj(p[objidx], NUM_POINT) for p in clusters], axis=0
        )  # use env to do filtering
