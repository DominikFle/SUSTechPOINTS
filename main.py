...
logging.basicConfig(
    filename="./logs/sustechpoints.log",
...
def build_dataset_cfgs():
    ...
    dir_org = (
        datacfg["global"]["dirorg"] if "dirorg" in datacfg["global"] else "by_scene"
    )
    ...
    if dir_org == "by_scene":
        ...
        for d in [
            ...
            dataset_cfg[d] = (
                datacfg["global"][d] if d in datacfg["global"] else (root + "/" + d)
            )
...
class Api(object):
        ...
        if args.save == "yes":
            ...
            for d in data:
                ...
                if usercfg[userid]["readonly"] == "yes":
                    ...
                    return {
                        "result": "fail",
                        "cause": "saving disabled for current user",
                    }
    ...
    def check2dlabels(self, scene):
        ck = check_fusion.FusionLabelChecker(
            os.path.join(datacfg["global"]["rootdir"], scene)
        )
        ck.check()
        # print(ck.messages)
        return ck.messages
    ...
    def save_image_annotation(self):
        if args.save == "yes":
            ...
            dataset.save_iamge_annotation(
                scene, frame, d["cameraType"], d["cameraName"], d
            )
    ...
    def get_user_info(self):
        ...
        return {"annotator": userid, "readonly": readonly}
...
cherrypy.config.update("./conf/server.conf.example")
...
if authcfg["global"]["auth"] == "yes":
    ...
    root_config["/data"] = {
        ...
        "tools.etags.autotags": True,
        "tools.response_headers.on": True,
        "tools.response_headers.headers": [("cache-control", "public, max-age=604800")],
        "tools.check_file_access.on": True,
        "tools.check_file_access.default": True,
    }
