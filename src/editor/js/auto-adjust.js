...
import { transpose, matmul2, eulerAngleToRotationMatrix3By3, normalizeAngle } from './util.js';
...
function AutoAdjust (boxOp, mouse, header) {
  ...
  this.followStaticObjects = function (box) {
    ...
    const staticObjects = world.annotation.boxes
      .filter(b => b !== box && b.obj_attr && b.obj_attr.search('static') >= 0)
      .map(refObj => {
        const coord = eulerAngleToRotationMatrix3By3(refObj.rotation);
        const trans = transpose(coord, 3);
        const p = [box.position.x - refObj.position.x,
        ...
        const distance = Math.sqrt(relativePos[0] * relativePos[0] + relativePos[1] * relativePos[1] + relativePos[2] * relativePos[2]);
        return {
          obj_id: refObj.obj_id,
          relativePos,
          relativeRot,
          distance
    ...
    const worldList = box.world.data.worldList;
    // let saveList = [];
    worldList.forEach(w => {
      if (w === box.world) {
        // current frame
      ...
      const existedBox = w.annotation.boxes.find(b => b.obj_id === box.obj_id);
      ...
      let candPoseSets = staticObjects.map(refObj => {
        const refObjInW = w.annotation.boxes.find(b => b.obj_id === refObj.obj_id);
        if (!refObjInW) {
          // not found refobj in this world, give up
          return null;
        }

        const relativePos = refObj.relativePos;
        const relativeRot = refObj.relativeRot;

        const coord = eulerAngleToRotationMatrix3By3(refObjInW.rotation);

        const rp = matmul2(coord, relativePos, 3);
        const newObjPos = {
      ...
      const denorm = candPoseSets.reduce((a, b) => a + b.weight, 0);

      const newObjPos = { x: 0, y: 0, z: 0 };
      const newObjRot = { x: 0, y: 0, z: 0, cosZ: 0, sinZ: 0 };
      ...
      if (window.pointsGlobalConfig.ignoreDistantObject) {
        const objDistance = Math.sqrt(newObjPos.x * newObjPos.x + newObjPos.y * newObjPos.y + newObjPos.z * newObjPos.z);
      ...
      if (existedBox) {
        ...
        const newBox = w.annotation.addBox(newObjPos,
          box.scale,
          newObjRot,
          box.obj_type,
          box.obj_id,
  ...
  this.followsRef = function (box) {
    ...
    const world = box.world;
    const refObj = world.annotation.boxes.find(b => b.obj_id === this.marked_object.ann.obj_id);
    if (refObj) {
  ...
  this.syncFollowers = function (box) {
    const world = box.world;
    const allFollowers = world.annotation.boxes.filter(b => b.follows && b.follows.obj_id === box.obj_id);
    ...
    allFollowers.forEach(fb => {
      const relpos = [fb.follows.relative_position.x,
        fb.follows.relative_position.y,
        fb.follows.relative_position.z
      ];

      const rp = matmul2(coord, relpos, 3);
  ...
  this.paste_bbox = function (pos, addBox) {
    if (!pos) { pos = this.marked_object.ann.psr.position; } else { pos.z = this.marked_object.ann.psr.position.z; }

    return addBox(pos, this.marked_object.ann.psr.scale, this.marked_object.ann.psr.rotation,
      this.marked_object.ann.obj_type, this.marked_object.ann.obj_id, this.marked_object.ann.obj_attr);
  };

  // this.auto_adjust_bbox=function(box, done, onBoxChanged){

  //     saveWorld(function(){
  //         do_adjust(box, onBoxChanged);
  //     });
  //     let _self =this;
  //     function do_adjust(box, onBoxChanged){
  ...
  //                             "object_id=" + marked_object.obj_id + "&" +
  ...
  this.smart_paste = function (selectedBox, addBox, onBoxChanged) {
    let box = selectedBox;
    if (!box) {
      const sceneP = this.mouse.getMouseLocationInWorld();
      // trans pos to world local pos
      const pos = this.data.world.scenePosToLidar(sceneP);
      box = this.paste_bbox(pos, addBox);
