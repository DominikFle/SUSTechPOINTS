...
import { saveWorldList, reloadWorldList, installSaveCallback } from './save.js';
...
function BoxEditor (parentUi, boxEditorManager, viewManager, cfg, boxOp,
  ...
  this.setSelected = function (selected, eventId) {
    if (selected) {
      ...
      if (this.box && this.boxEditorManager.objectTrackView.visible()){
        this.boxEditorManager.objectTrackView.highlightOneTrack(this.box.world.frameInfo.frame);
      }     
      

    } else {      
      if (this.box && this.boxEditorManager.objectTrackView.visible()){
        this.boxEditorManager.objectTrackView.unhighlightOneTrack(this.box.world.frameInfo.frame);
      }
  ...
  this.attachBox = function (box) {
    ...
    if (box) {
      ...
      if (this.isInBatchMode()) {
        ...
        this.boxEditorManager.onBoxAttached(this);
      }
...
function BoxEditorManager (parentUi, viewManager, objectTrackView,
  ...
  this.funcOnBoxChangedMgr=(box)=>{
    funcOnBoxChanged(box);
    this.updateTrajectoryWindow(box);

    installSaveCallback('check', ()=>{
      check3dLabels(this.editingTarget.sceneMeta.scene, this.editingTarget.objTrackId);
    })
  ...
  this.delayUpdateAutoGeneratedBoxesType = 'pseudo';

  this.updateAutoGeneratedBoxes = function (boxtype) {

    if (boxtype) {
      this.delayUpdateAutoGeneratedBoxesType = boxtype;
    } else {
      this.delayUpdateAutoGeneratedBoxesType = 'all';
    }
    ...
    this.delayUpdateAutoGeneratedBoxesTimer = setTimeout(async () => {
      if (this.cfg.autoUpdateInterpolatedBoxes && this.delayUpdateAutoGeneratedBoxesType !== 'pseudo') {
  ...
  this.onBoxAttached = function(e) {
    if (this.isCheckingFrameMode()) {
      return;
    }

    this.updateAutoGeneratedBoxes('pseudo');
  }
    ...
    this.getSelectedEditors().forEach(e => {
      if (e.box) {
        if (e.box.annotator) {
          ...
          this.funcOnBoxChangedMgr(e.box);
  ...
  this.editAttributes = function(){
    const box = this.firingBoxEditor.box;
    if (box) {
      this.firingBoxEditor.editAttributes();
    }
  };


  this.interpolateAllOthers = function (infoBoxPos) {
    let targetEditors = this.activeEditorList().filter(e => e.box && e !== this.firingBoxEditor);
    ...
    let funcDo = () => {
      ...
      let applyIndList = this.activeEditorList().map(e => true);
      this.interpolate(applyIndList, 'ignoreEmpty');
  
      this.updateAutoGeneratedBoxes();
    };


    if (targetEditors.length >= 2) {
      window.editor.infoBox.show(
        'Confirm',
                `Delete <span class="red">${targetEditors.length}</span> other boxes and re-interpolate?`,
                ['yes', 'no'],
                (btn) => {
                  if (btn === 'yes') {
                    funcDo();
  ...
  this.handleContextMenuEvent = function (event) {
    ...
    switch (event.currentTarget.id) {
      ...
      case 'cm-interpolate':
        this.interpolateSelectedFrames();
        break;
      case 'cm-interpolate-all-others':
        this.interpolateAllOthers({ x: event.clientX, y: event.clientY });
        break;
      case 'cm-edit-attr':
        this.editAttributes();
        break;
      ...
      case 'cm-fit-moving-direction':
        this.getSelectedEditors().forEach(e => {
          ...
          if (estimatedRot) {
            ...
            this.funcOnBoxChangedMgr(currentBox);
          }
        });
      ...
      case 'cm-fit-position':
        this.getSelectedEditors().forEach(e => {
          ...
          boxOp.auto_rotate_xyz(e.box, null,
            ...
            this.funcOnBoxChangedMgr, // onBoxChangedInBatchMode,
      ...
      case 'cm-check':
        {
          check3dLabels(this.editingTarget.sceneMeta.scene);
          logger.show();
  ...
  this.interpolate = async function (applyIndList, ignoreEmpty=false) {
  ...
  this.showTrajectory = (highlightBox) => {
    ...
    const tracks = this.editingTarget.data.worldList.map(w => {
      ...
      return [w.frameInfo.frame, ann, box != null && box===highlightBox];
    });
  ...
  this.updateTrajectoryWindow = function (box){
    if (this.objectTrackView.visible()) {

      if (this.updateTrajectoryTimer) {
        clearTimeout(this.updateTrajectoryTimer);
      }
    
      this.updateTrajectoryTimer = setTimeout(() => this.showTrajectory(box), 200) 
    }
  }
  ...
  this.allocateEditor = function () {
    if (this.activeIndex >= this.editorList.length) {
      const editor = new BoxEditor(this.boxEditorGroupUi, this, this.viewManager, cfg, this.boxOp, this.funcOnBoxChangedMgr, funcOnBoxRemoved, String(this.activeIndex));
