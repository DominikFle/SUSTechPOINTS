...
parser = argparse.ArgumentParser(description='extract 3d objects')        
...
def crop_pts(pts, box):
    ...
    whole_body_filter = filter & (box_pts[:, 2] < box['psr']['scale']['z']/2)  & (box_pts[:, 2] > - box['psr']['scale']['z']/2)

    return [pts[topfilter], pts[groundfilter], pts[whole_body_filter], box_pts[whole_body_filter]]
...
def proc_one_frame(scene, frame, calib):
    ...
    for obj in objs:
        ...
        print(obj_pts.shape)
        print(frame, box['obj_type'], box['obj_id'])
        obj_pts.tofile("./data/temp/lidar/{}.bin".format(frame+"-"+box['obj_type']+'-'+str(box['obj_id'])))
...
def proc_one_scene(scene):
    ...
    for frame in frames:
        if re.fullmatch(args.frames, frame):
            proc_one_frame(scene, frame, calibs)
